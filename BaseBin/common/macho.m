...
int64_t machoFindArch(FILE *machoFile, uint32_t subtypeToSearch)
{
	...
	machoEnumerateArchs(machoFile, ^(struct fat_arch* arch, uint32_t archMetadataOffset, uint32_t archOffset, BOOL* stop) {
		...
		NSLog(@"offset: %d, origin cpusubtype: %x, host endian subtype: %x, masked subtype: %x, subtype to search %x", archOffset, mh.cpusubtype, OSSwapLittleToHostInt32(mh.cpusubtype), maskedSubtype, subtypeToSearch);
...
#if __arm64e__
#define __CR_arm64e__ true
#elif __arm64__
#define __CR_arm64e__ ({const NXArchInfo *info = NXGetLocalArchInfo(); info->cputype == CPU_TYPE_ARM64 && (info->cpusubtype & CPU_SUBTYPE_ARM64E) == CPU_SUBTYPE_ARM64E;})
#endif
