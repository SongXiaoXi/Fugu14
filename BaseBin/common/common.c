...
#include <sys/stat.h>
#include <sys/mount.h>
#include "../../arm/iOS/jailbreakd/xpc/xpc.h"
#include "../../arm/iOS/jailbreakd/xpc/private.h"
#include <objc/objc.h>
#include <errno.h>
#include <paths.h>
...
int64_t jbdswProcessBinary(const char *filePath) {
	...
	xpc_dictionary_set_string(message, "CMD", JAILBREAKD_CMD_TRUSTCACHE_INJECT);
...
int64_t jbdswDebugMe(void)
{
	...
	xpc_object_t message = xpc_dictionary_create_empty();
	xpc_dictionary_set_string(message, "CMD", JAILBREAKD_CMD_DEBUG_ME);
	xpc_object_t reply = sendJBDMessageSystemWide(message);
	int64_t result = -1;
	if (reply) {
		result  = xpc_dictionary_get_int64(reply, "RET");
...
int64_t jbdswFixSetuid(void) {
	xpc_object_t message = xpc_dictionary_create_empty();
	xpc_dictionary_set_string(message, "CMD", JAILBREAKD_CMD_SETUID);
	xpc_object_t reply = sendJBDMessageSystemWide(message);
	int64_t result = -1;
	if (reply) {
		result  = xpc_dictionary_get_int64(reply, "RET");
...
int64_t jbdswForkFix(pid_t childPid, bool mightHaveDirtyPages) {
	xpc_object_t message = xpc_dictionary_create_empty();
	xpc_dictionary_set_string(message, "CMD", JAILBREAKD_CMD_FORK_FIX);
