...
static bool unsandbox(void) {
	FILE *fp = fopen("/usr/lib/unsandbox.txt", "rb");
	if (fp == NULL) {
		return false;
	}
	char line[4096];
  	...
  	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		sandbox_extension_consume(line);
  	}
	fclose(fp);
	return true;
}
...
initializer(void) {
	...
	freeExecutablePath();
	if (strcmp(getenv("DYLD_INSERT_LIBRARIES"), "/usr/lib/systemhook.dylib") == 0) {
